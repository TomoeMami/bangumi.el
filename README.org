...
[2026-07-31 周五 14:12]：请使用 2025-03-19 之后的 =plz= ，其内置了对 =PATCH= 方法的支持，对应版本号为 =0.10-pre= 。
...
(use-package bangumi :demand t
  :vc (:url "git@github.com:TomoeMami/bangumi.el"
            :rev :newest))
#+end_src
...
以上映射由函数 =bangumi-update-subject-status= 完成。若您使用自定义的 =org-todo-keywords= ，覆盖该函数即可让插件按您自己的关键词判断收藏状态：

#+begin_src elisp
(defun bangumi-update-subject-status (to-state)
  "将 TODO 关键字 TO-STATE 映射为 Bangumi 收藏状态 (TYPE . NAME)。"
  (pcase to-state
    ("WATCHING" '(3 . "在看"))
    ("FINISHED" '(2 . "看过"))
    ("PLANNED"  '(1 . "想看"))
...
这样无需修改插件本体，即可让 =TODO= / =DONE= 之外的关键词（如 =WATCHING= 、 =FINISHED= ）也能正确同步。
...
#+begin_src elisp
bangumi-update-subject-status TO-STATE
#+end_src

将 TODO 关键字 =TO-STATE= 映射为 Bangumi 收藏状态的函数，返回形如 =(TYPE . NAME)= 的 cons。原本写死在 =bangumi-update-subject= 中的判断被提取到这里，默认支持 =TODO= / =DONE= / =HOLD= / =XXXX= ，其余值（包括无 TODO 关键字）视为搁置。

用户可以覆盖此函数，以适配自己的 =org-todo-keywords= ，详见上文“更新观看状态”一节。
